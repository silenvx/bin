...
echo "${1}"|grep -E 'http://seiga.nicovideo.jp/comic/[0-9]+' > /dev/null 2>&1
if [ "${?}" == '1' ];then
...
for comic_episode in `echo "${comic_source}"|grep '<div class="episode"'`;do
    comic_episode_url="http://seiga.nicovideo.jp`echo "${comic_episode}"|grep -E -o '<a href="[^"]*'|head -1|sed -e 's/<a href="//'`"
    comic_episode_source="`nicovideo_fetch -O - "${comic_episode_url}"`"
    comic_episode_title="`echo "${comic_episode_source}"|grep -E -o '<h1><[^<]*'|sed -e 's/[^>]*>[^>]*>//'`"
    comic_episode_id="`echo "${comic_episode_url}"|sed -e 's|.*nicovideo.jp/watch/||' -e 's/?.*//'`"
    comic_episode_number="`echo "${comic_episode}"|grep -E -o 'data-number="[^"]*'|grep -E -o '[0-9]*'`"
    for comic_episode_image in `echo "${comic_episode_source}"|grep -E -o 'http:\/\/lohas.nicoseiga.jp\/thumb\/[0-9]*p'`;do
